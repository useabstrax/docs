...
The username argument is validated using the same rules as the [user commands](users.md#username-validation).
...
Before reloading, the configuration is validated with `sshd -t` (when `sshd` is available). The managed include file is backed up with a timestamped suffix before it is rewritten (see [Security](../reference/security.md)).
...
- [Users](users.md)
- [Firewall](system.md#firewall-commands)
